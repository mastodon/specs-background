...
- expose the user (with an introduction) to the boosters followers in order to build community
- add a personal comment or anecdote to someone else's post
- debunk some piece of information
...
A while ago, we asked a former Twitter developer for their opinion on Quote Tweets, what they learnt, and what they would do differently. They have a great writeup at <https://mergesort.craft.me/notes-on-quotes>

### Tumblr

On Tumblr, quote posts are Tumblr Reblogs and are documented at <https://help.tumblr.com/reblogs/>

On Tumblr reblogs, any additional comment is added at the end of the post, with the quoted content at the top.
...
Bluesky quote posts are very similar to Quote Tweets, the comment come before the quote, and onlookers get access to a list of quotes.
...
Bluesky (at least the main client+AppView) also forbids you from authoring posts merely linking at another post that has quotes disabled.

In Bluesky's design, the Quote Posts still exist in the PDS of the author of the Quote Post, but are filtered/removed by the AppView. This means both the quote post and its denial are fully public, and the former are filtered relative to the latter in the main user-facing interfaces.
...
Quote controls are defined in `app.bsky.feed.postgate` entities <https://github.com/bluesky-social/atproto/blob/main/lexicons/app/bsky/feed/postgate.json> analogous to the `threadgate` documented here <https://docs.bsky.app/docs/tutorials/thread-gates>
...
The documentation hints that quote posts are using FEP-e232.
...
Conversely, when used to denounce hateful speech, this participates in spreading the hateful speech further and building “more engagement”.
